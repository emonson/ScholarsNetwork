...
def convert_embedded_dates_remove_citations(obj, convert=False):
    ...
    if convert and isinstance(obj, basestring):
        ...
        return dict((k, convert_embedded_dates_remove_citations(v, (k.lower().endswith('date') or k.lower().endswith('year')))) for k, v in obj.items() if not k.lower().endswith('citation'))
        
    elif isinstance(obj, (list, tuple)):
        return map(convert_embedded_dates_remove_citations, obj)
...
if __name__ == "__main__":
    ...
    r = requests.get('https://scholars.duke.edu/widgets/api/v0.9/people/complete/all.json?uri=https://scholars.duke.edu/individual/per8008362')
    print r.status_code
    c = json.loads(r.content)
    c_new = convert_embedded_dates_remove_citations(c)
