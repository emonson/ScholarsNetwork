...
var load_nodes = function() {
    d3.csv('data/forum_forum_norm_bin_ord_nodePos.csv', depts_accessor, load_edges);
};
...
var load_edges = function(node_data) {
    ...
    for (var ii=0; ii<node_data.length; ii++) {
        ...
        author_names.push(node.forum);
        author_name_id_dict[node.forum] = node.id;
    }
    graph.nodes = node_data;
    author_names = author_names.sort();
    
    // Actually start loading edge data
    d3.csv('data/forum_forum_norm_bin_eges.csv', edges_accessor, init_vis);
};
...
var init_vis = function(edge_data) {
    ...
    var nodes = svg.selectAll(".node")
        ...
        .enter()
            ...
            .attr("class", "node")
            .attr("r", function(d) {return d.r/10.0;})
            .style("fill", function (d) { return d.component_id == 0 ? color(d.mod_class) : "gray"; })
            .call(force.drag)
...
var tip = d3.tip()
    .attr('class', 'd3-tip')
    .offset([-8, 0])
    .html(function (d) { return  "<strong>"+ d.forum + "</strong>"; });
svg_base.call(tip);
    ...
    info_panel.selectAll(".info_data_element")
        ...
        .enter()
            ...
            .append("div")
                ...
                .text(function(d){ return d.forum; })
            .append("div")
                .attr("class", "info_data_element_circle")
                .attr("id", function(d){ return "in_" + d.id; })
                .style("background-color", function(d){return color(d.mod_class);});
...
function clicked(d) {
  ...
  document.getElementById('search').value = d.forum;
